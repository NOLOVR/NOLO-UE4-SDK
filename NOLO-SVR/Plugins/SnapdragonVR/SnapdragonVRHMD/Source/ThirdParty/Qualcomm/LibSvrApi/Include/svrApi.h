...
*/
typedef enum {
	SVR_PoseOriginModel_OriginOnHead = 0, /**< The origin of 6 DoF pose is on head. */
	SVR_PoseOriginModel_OriginOnGround = 1, /**< The origin of 6 DoF pose is on ground. */
	SVR_PoseOriginModel_OriginOnTrackingObserver = 2, /**< The raw pose from tracking system. */
	SVR_PoseOriginModel_OriginOnHead_3DoF = 3, /**< The origin of 3 DoF pose is on head. */
} SVR_PoseOriginModel;
enum SvrResult {
...
*/
typedef enum {
	SVR_Eye_Left = 0,                                          /**< WVR_Eye_Left: Left eye id. */
	SVR_Eye_Right = 1,                                         /**< WVR_Eye_Right: Right eye id. */
} svrEye;
...
static void L_CreateLayout(float centerX, float centerY, float radiusX, float radiusY, svrLayoutCoords *pLayout)
{
...
extern "C" {
	...
	//! \brief Calculates a predicted head pose without using head parameters
	//! \param predictedTimeMs Time ahead of the current time in ms to predict a head pose for
	//! \return The predicted head pose and relevant pose state information
	SVRP_EXPORT svrHeadPoseState svrGetPredictedHeadPoseModify(float predictedTimeMs);
	...
	//! \brief Retrieves point cloud data. Each point has three float components: XYZ
	SVRP_EXPORT SvrResult svrGetPointCloudData(uint32_t *pNumPoints, float **pData);
	...
	SVRP_EXPORT int
		svrControllerQuery(int controllerHandle, int what, void *memory, unsigned int memorySize);
